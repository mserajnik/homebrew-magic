...
# The branch's current tip on the remote, passed as the mutation's
# `expectedHeadOid`. It is read fresh here, so the commit lands on the current
# tip (last writer wins) and only rejects if the tip moves in the brief window
# before the mutation. This is safe because the casks are bot-managed and build
# runs are serialized by the workflow's concurrency group.
